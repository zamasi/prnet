...
class PRN:
    ...
    def __init__(self, is_dlib = False, prefix = '.'):
        ...
        if is_dlib:
            ...
            self.face_detector = dlib.cnn_face_detection_model_v1(
                    detector_path)#最终还是调用cnn模型来进行预测
    ...
    def generate_uv_coords(self):#转换成3D？？
    ...
    def process(self, input, image_info = None):#此处开始处理
        ...
        #*矩阵补充维数
        #此处开始检测人脸
        if image_info is not None:#如果有图像信息就不重新检测左右上下与中心五个顶点
        ...
        # crop image将图片中的人脸裁剪出来
        src_pts = np.array([[center[0]-size/2, center[1]-size/2], [center[0] - size/2, center[1]+size/2], [center[0]+size/2, center[1]-size/2]])
        DST_PTS = np.array([[0,0], [0,self.resolution_inp - 1], [self.resolution_inp - 1, 0]])
        tform = estimate_transform('similarity', src_pts, DST_PTS)
        
        image = image/255.
        cropped_image = warp(image, tform.inverse, output_shape=(self.resolution_inp, self.resolution_inp))#裁剪后的像素

        # run our net
        # st = time()
        cropped_pos = self.net_forward(cropped_image)#裁剪后的坐标，这个是UV坐标吗？
    ...
    def get_landmarks(self, pos):#地标就是骨骼坐标？#此处开始使用UV位置映射
        '''
        Args:
    ...
    def get_vertices(self, pos):
        ...
        Args:
            pos: the 3D position map. shape = (256, 256, 3).
        Returns:
            vertices: the vertices(point cloud). shape = (num of points, 3). n is about 40K here.3D点云
        '''
