...
def main(args):
    ...
    for i, image_path in enumerate(image_path_list):
        ...
        if args.isDlib:
            ...
            pos = prn.process(image) # use dlib to detect face获取人脸并裁剪出来
        else:
            if image.shape[0] == image.shape[1]:
                image = resize(image, (256,256))#对图像大小进行改变
                pos = prn.net_forward(image/255.) # input image has been cropped to 256x256对图像进行裁剪
            else:
                box = np.array([0, image.shape[1]-1, 0, image.shape[0]-1]) # cropped with bounding box与预测盒子一起裁剪
